...
*** Holo Hosting Receipts 
    ...
    |------+-----------+---------------------------------------------------------------------------------|
    | 5.88 | 5.06+0.82 | 1.0000 holo Host, 0.0340 net TB, 0.0902 ram GB, 0.0100 data TB, 0.0350 cpu Core |
    | 3.74 | 4.14-0.41 | 1.0000 holo Host, 0.0179 net TB, 0.0577 ram GB, 0.0083 data TB, 0.0308 cpu Core |
    | 3.60 | 4.37-0.77 | 1.0000 holo Host, 0.0133 net TB, 0.0908 ram GB, 0.0092 data TB, 0.0715 cpu Core |
    | 3.56 | 3.38+0.18 | 1.0000 holo Host, 0.0020 net TB, 0.0416 ram GB, 0.0065 data TB, 0.0389 cpu Core |
    | 5.13 | 4.79+0.35 | 1.0000 holo Host, 0.0251 net TB, 0.0557 ram GB, 0.0105 data TB, 0.0778 cpu Core |
    |  ... |           |                                                                                 |
    :END:
    #+LATEX: }
...
*** Recovery of Commodity Valuations
    ...
    | Commodity   | Predicted | Actual |   Error |
    |-------------+-----------+--------+---------|
    | holo        |      2.65 |   2.67 | -0.484% |
    | net         |     42.33 |  42.45 | -0.273% |
    | ram         |      4.47 |   4.55 | -1.803% |
    | data        |     34.03 |  33.31 | +2.144% |
    | cpu         |      5.99 |   5.83 | +2.899% |
    :END:
...
*** Simple Host/dApp Pricing Model
    ...
    |-------------------------+-------------+------------+------------+-----------+------------|
    | 25421.131991            | 5977.763608 | 119.279426 | 237.151330 | 58.966540 | 237.002337 |
    |-------------------------+-------------+------------+------------+-----------+------------|
    | Score(R^2) 0.9867390    |             |            |            |           |            |
    | Predicted               |        2.62 |      42.36 |       4.97 |     34.37 |       6.26 |
    | Actual                  |        2.67 |      42.45 |       4.55 |     33.31 |       5.83 |
    | Error                   |     -1.777% |    -0.205% |    +9.204% |   +3.192% |    +7.473% |
    |-------------------------+-------------+------------+------------+-----------+------------|
    | Actual  Basket          |      106.55 |            |            |           |            |
    | Predict Basket          |      106.45 |            |            |           |            |
    | Error                   |     -0.101% |            |            |           |            |
    :END:
...
*** Simple Credit Feedback Control
    :PROPERTIES:
    :ID:       F2384A02-DE6B-47C6-BA84-DAD4E28FED7F
    :END:
    ...
    #+BEGIN_SRC ipython :session :file images/receipts-regress-hourly-PID.png :exports both
      ...
      credit                  = credit_sine_prices_pid_K( 
				       ...
				       price_initial=credit.value() ) # Est. initial price --> infllation
      ...
      fig,(ax1,ax2,ax3,ax4,ax5)= plt.subplots( 5, sharex=True, figsize=(6,6) )
      ax1.plot( [ x for x,P in credit.price_curr_trend ], [ P for x,P in credit.price_curr_trend ],
               "g-",    label="Price (Feedback)" )
      ...
      plt.xlim( 0.01, 5.0 )
      ax1.set_title( "Hourly Inflation Stabilization (%d Receipts/hr.)" % ( hosts_count ))
      plt.show()
      print( "done" )
