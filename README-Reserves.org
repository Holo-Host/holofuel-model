...
* Host Currency Preference and Minimum Cash-Out Value
  ...
  #+LATEX: {\scriptsize
  #+BEGIN_SRC ipython :session :ipyfile images/currency-pie.png :exports both :results raw drawer
...
**** Fees
     ...
     |    1% | < ~USD$100    | ~USD$0.10  | Microtransaction and small retail                      |
     ...
     space is critical to the uptake of Holo fuel, and capping the fees at ~USD$0.10 (10 cents) the
     Holo fuel equivalent of < ~USD$100.00 purchases will be acceptable, and will cover the vast
     majority of transactions.  The only transactions that will produce reduced Holo system fees are
     those between ~USD$10.00-100.00.  Instead of the USD$1.00 fee on a $100 purchase, Holo would
     earn USD$0.10.  These transactions are in the gray area where existing Cryptocurrencies have
     similar fees on this scale of transactions; this cap would establish Holo fuel as a clear
     winner against existing Cryptocurrencies, and every Fiat currency payment processing system.

     Transactions between ~USD$100-$10,000.00 are the bulk of larger consumer retail transactions,
     ...
     USD$100-$10,000 and capping to a max USD$1.00, the fee ceases to be a negative psychological
     factor in these purchases; Holo fuel will be the lowest cost option available for retail level
     ...
     find a 0.1% fee on *all* their earnings, payroll and supplier payments significant; A small
     business that earns USD$1,000,000/yr would bear an additional USD$1,000 expense due to
     accepting Holo fuel.  This would be noticeable to accountants and business owners, and would
     affect uptake negatively.  By limiting this to .01% and capping fees at USD$1,000, the fees on
     USD$10,000-10,000,000 would be USD$1-1,000 on large transactions; roughly comparable to the
     cost in annual fees for a business bank account.  This would ameliorate any objections to Holo
     fuel uptake at the boardroom level.  Once again, a 1% or 0.1% fee would be a non-starter, so
     any losses in fee income to Holo are imaginary.
...
**** Multiple Degrees of Freedom on Holo fuel Valuation
    ...
    - The price of a unit of Holo fuel in various Fiat currencies (eg. exchange rate to USD$)
      - What will I earn when I sell that fuel on the Reserves or other exchanges?
        - Defined by what the market believes Holo fuel is "worth"; (Utility + Purchasing Power) / Supply
    ...
    arrived at by the market reaching equilibrium.  What everyone *does* know, however, is how much 1
    ...
    of that Holo fuel per year (Holo hApps and Hosts only saving about 1 month's worth), the compute
    ...
    and against deposits (eg. via Reserves).  Even if that target value is low (say, USD$0.01 or
    USD$1.77 billion, and if the Holo system generated USD$5.00 per core per month in revenue, the
    NPV of one year's worth of 1 core worth (USD$60) of hosting at a 10% discount rate is USD$54.55.
    Estimating a credit factor of 50%, that results in USD$27.27 of credit extended per
    core. Extending approximately the same amount of credit as the valuation of the already issued
    tokens would require about 65 million cores of Holo Hosting power online; not immediately
    practical.
...
*** Modelling Holo Fuel
    ...
    #+LATEX: {\scriptsize
    #+BEGIN_SRC ipython :session :ipyfile images/client-requests.png :exports both :results raw drawer
...
plt.show()
    ...
    #+LATEX: {\scriptsize
    #+BEGIN_SRC ipython :session :ipyfile images/client-holdings.png :exports both :results raw drawer
